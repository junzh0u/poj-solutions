...
Only the submit itself needs the user's Chrome session (logged in as user `150014`). Everything else — the statement, the samples, the verdict — is plain `curl` with no login, so the browser is touched exactly once per submission: open a fresh tab when ready, keep it alive for whatever post-click landing window the transport needs, then close it and do every status read with `curl`. Whether the POST is already handed off when the JavaScript call returns is transport-specific — measured, not assumed — so the harness note is authoritative: claude-in-chrome closes on the next call, the AppleScript path holds the tab five seconds. A shorter browser window means fewer chances for a sibling agent to re-navigate the tab, but the in-call guard in step 3 stays regardless.
...
The JavaScript call returning is not by itself proof that the browser handed off the POST — on some transports it is, on others it is not. Follow the harness transport note's post-click landing window rather than guessing, and where a note claims a window is unnecessary, that claim should name the submission that demonstrated it. After the window, close the tab and move all confirmation and verdict reads back to `curl`.
