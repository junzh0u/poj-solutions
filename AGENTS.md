...
## The solve agent's job

Every solve runs in a **subagent** of an orchestrating parent — the statement, the test scaffolding, and the failed attempts stay out of the parent's context, which only ever sees the report. The agent owns steps 1–6 below for exactly one problem, ending at **Accepted** or a hand-back, and never touches shared state: no git, no `TODO`, no `attempts/` — concurrent `git add`/`git commit` in one worktree races on `index.lock` and can sweep a sibling's files into the wrong commit. Everything downstream of the report is "The parent's job" below.
...
- **Test data that earned its keep goes to `solutions/<id>/test_data/`** rather than dying in the scratchpad — official contest data found online, a separating case that settled an ambiguity, a stress input that caught a real bug. The solve agent writes the files (labelled per the Layout section); they ride the parent's commit alongside the source or the park note. Data that merely echoed the statement's sample or never disagreed with anything stays out.
...
The parent has already preflighted the login and pinned the browser target (see "The parent's job"); use exactly that target for every attempt and retry, never one that merely looks available. POJ sessions still expire mid-run: on a logged-out page the login form replaces everything and `document.forms[2]` does not exist (`document.forms.length` is 1). The snippet's guard catches that before any source is planted; when it fires, report back rather than wait or work around it — only the user can log back in, the loss is an infrastructure interruption, and no attempt is consumed.
...
Cap the run at **5 submissions** — except on a stronger-model retry of a parked problem, which runs under **10**, extendable to 20 only while each further submission tests a genuinely distinct hypothesis. A fresh solve past 5 is usually grinding a wrong approach, but a park already proves the cheap failure modes are exhausted, and the retry exists to spend attempts the first run could not; blind permutation of axes the park note already swept stays out of bounds at any cap.

### 6. Hand off

Only after **Accepted**, copy the exact submitted source byte-for-byte to `solutions/<id>/<runId>_AC_<time>MS_<mem>K.<ext>`. Its title, model, approach, and ambiguity comment block was finalized and tested before submission; do not add or edit it after the verdict.

The final report is everything the parent acts on without redoing any part of the solve: every submission (run id, verdict, code length, what changed between attempts) and the `// Model:` line verbatim; on Accepted, the archived path with the run's time and memory, plus any test_data files saved; on a cap hit or a stall before submitting, the last verdict, what was tried, and what the problem actually needs — the parent turns that into the `attempts/<id>.md` park.

An accepted report ends with the **commit body**, replacing the corresponding prose rather than sitting beside it — the parent composing it from the report would be authoring the same content a second time from a worse vantage point. Its form: plain text, no markdown headings or tables, no HTML entities, prose wrapped at the usual width, about twenty lines, ending with `Model: <model-id>` matching the source comment. Content is the algorithm and the decisions behind it — the statement ambiguity and how it was settled, what the board contributed and whether it was checked or merely believed, what the verification actually established — never a narration of the code.

## The parent's job

The parent owns everything shared: browser preflight, submit coordination, commits, `TODO`, and `attempts/`. It runs a pool of **N solve subagents** — five by default, and a single named problem is simply N=1 with the same preflight, pinned browser, and commit path, not a different procedure. Spawn the first N one per problem, all in a single message so they start together; from then on, spawn a replacement from the top of `TODO` as soon as any one agent reports, without waiting for its siblings.
...
The parent commits each accept as its agent reports, one problem per commit — in whatever order the verdicts arrive, not `TODO` order; ordering the commits means holding a finished solve hostage to a slower sibling, and buys nothing, because the commits are independent and `TODO` is a set, not a sequence. Subject is `<id> <Title>` — plain, no Conventional Commits prefix; the body is the one the agent's report carries (§6), ending with `Model: <model-id>` matching the source comment. Once the commit lands, the parent strikes the id from `TODO`.
