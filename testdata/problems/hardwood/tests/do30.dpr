...
begin
	...
	while true do begin
		...
		case random(3) of
        	...
        	1: begin
        		repeat
        			...
        			y1 := random(n) + 1;
        			y2 := random(n) + 1;
        		until (x1 <= x2) and (y1 <= y2) and (x1 > 1) and (x1 < m) and (y1 > 1) and (y1 < n) and
        			((b[x1 - 1][y1 - 1] = b[x1][y1]) and (b[x2][y2] = b[x2 + 1][y2 + 1]));
        	end;
        	2: begin
        		repeat
        			x1 := random(m) + 1;
		...
		if (cc > length(s)) and (oldcc > length(s) - 5) then break;
		if cc > length(s) then
			b := saveb;
	end;
	b := saveb;
