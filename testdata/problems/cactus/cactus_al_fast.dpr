...
procedure muls (var a:long; s:extended);
var i:integer;
    cy, nc:extended;
begin
  cy:=0;
  for i:=1 to a[0]+1 do begin
    cy:=a[i]*s+cy;
    nc:=int (cy*InvBase+eps);
    a[i]:=round (cy-nc*Base);
    cy:=nc;
  end;
...
begin
  write (a[a[0]]);
  for i:=a[0]-1 downto 1 do write (format ('%.9d', [a[i]])); writeln;
end;
...
begin
  ...
  a[0]:=1; a[1]:=1; curm:=1;
  for i:=1 to cc do 
    if curm*z[i]>1000000000 then begin
      muls (a, curm); curm:=z[i]
    end else curm:=curm*z[i];
  muls (a, curm);
  writel (a);
end.