...
Give every solve agent the exact preflighted window id, but do not pre-create its submission tab. Only after the annotated source and guarded payload are ready does the owning agent create a retained-ID background tab in that window, restore the user's prior active-tab index, verify that exact tab is ready, and report its unique id to the parent. Require the pinned window id for every attempt, execute the guarded JavaScript against the reported tab specifier, and close only that tab after the submission helper confirms completed navigation. Never tell an agent to find an existing or frontmost Chrome window for itself.
...
2. Build the guarded payload before opening a submission tab. When ready to submit, save the pinned window's active-tab index, open `http://poj.org/submit?problem_id=<id>` as a retained-ID background tab in that exact window, and restore the saved active-tab index immediately. Wait for that exact tab's `loading` property to become false, then run a read-only JavaScript check against the exact tab id and require its URL, `document.forms.length === 3`, and `document.forms[2].problem_id.value === '<id>'`. Do this before acquiring the submit lock on the first attempt and every retry. The canonical in-call guard remains mandatory because a readiness check is not atomic with the later click.
...
6. Before the first click and every retry, capture a fail-visible curl status baseline and record its newest Run ID, or explicitly record that it has no rows. After closing the retained submission tab, poll status per `AGENTS.md` §4 with curl and compare against that baseline. Preserve separate per-attempt before/after artifacts or transcript output instead of overwriting the only evidence. Do not keep or create a browser status tab merely for convenience.
...
Track the preflighted window id and each retained submit-tab id from the moment its owning agent reports creating it. After each solve agent exits, verify its tracked id is closed; an agent can finish before a queued cleanup message reaches it. The owning agent closes its exact retained tab immediately after the helper confirms that it finished loading POJ's status page. For parent cleanup, require the preflighted window id, retained tab id, and either the original problem submit URL or the expected POJ status landing URL; never close every POJ or submit tab indiscriminately. At the end of a cycle, require zero tracked tabs left open.
