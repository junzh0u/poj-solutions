...
5. Close the retained submission tab as soon as the guarded JavaScript call returns, even when AppleScript returns `missing value`, using the immediately following AppleScript statement. No foreground landing window is needed. Immediate next-statement closes delivered real POJ rows 25194713 and 25194714 with the expected code lengths; after an in-page blocked-event test proved the argv-safe helper reached `.click()` and preserved the exact encoded source, the same timing delivered Accepted run 25194746 at its expected 2,693-byte length.
